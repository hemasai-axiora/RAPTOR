...
class ContentPost extends Model {
    ...
    public function deleteMultiplePostsWithAudit(array $ids, string $remarks, int $userId, string $userName): int {
        $count = 0;
        foreach ($ids as $id) {
            $id = (int)$id;
            if ($id > 0 && $this->deletePostWithAudit($id, $remarks, $userId, $userName)) {
                $count++;
            }
        }
        return $count;
