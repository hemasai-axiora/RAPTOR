...
<!-- Top 3 Performers Showcase (3 Stacked Rows with 3 Columns: Rank, Name, Score) -->
<?php if (!empty($scores)): ?>
    <div class="pulse-card p-4 mb-4 shadow-sm" style="background: var(--panel-dark, #ffffff); border-radius: 16px; border: 1px solid var(--border-color, #e2e8f0);">
        ...
        <div class="d-flex flex-column gap-3">
            <?php
            $topRanks = [
                0 => ['title' => '🥇 #1 TOP PERFORMER', 'bg' => 'linear-gradient(135deg, rgba(245, 158, 11, 0.15), rgba(217, 119, 6, 0.04))', 'border' => '#f59e0b', 'badge_bg' => 'linear-gradient(135deg, #fbbf24, #f59e0b)', 'score_color' => 'text-warning', 'bar_bg' => 'bg-warning', 'icon' => '👑'],
                1 => ['title' => '🥈 #2 RUNNER UP', 'bg' => 'linear-gradient(135deg, rgba(148, 163, 184, 0.15), rgba(71, 85, 105, 0.04))', 'border' => '#94a3b8', 'badge_bg' => 'linear-gradient(135deg, #cbd5e1, #94a3b8)', 'score_color' => 'text-info', 'bar_bg' => 'bg-info', 'icon' => '🥈'],
                2 => ['title' => '🥉 #3 THIRD PLACE', 'bg' => 'linear-gradient(135deg, rgba(217, 119, 6, 0.12), rgba(180, 83, 9, 0.04))', 'border' => '#b45309', 'badge_bg' => 'linear-gradient(135deg, #d97706, #b45309)', 'score_color' => 'text-warning', 'bar_bg' => 'bg-warning', 'icon' => '🥉'],
            ];
            ?>
            <?php for ($idx = 0; $idx < 3; $idx++): ?>
                <?php if (isset($scores[$idx])): $p = $scores[$idx]; $cfg = $topRanks[$idx]; 
                    $bandStyle = match($p->performance_band) {
                ...
                ?>
                    <div class="p-3 rounded-3" style="background: <?php echo $cfg['bg']; ?>; border: 2px solid <?php echo $cfg['border']; ?>; border-radius: 14px;">
                        <div class="row align-items-center g-3">
                            <!-- Col 1: Rank -->
                            <div class="col-md-3">
                                <span class="badge px-3 py-2 fw-bold text-dark w-100 text-center" style="background: <?php echo $cfg['badge_bg']; ?>; font-size: 0.88rem; border-radius: 8px;">
                                    <?php echo $cfg['title']; ?>
                                </span>
                            </div>
                            <!-- Col 2: Employee Name & Team -->
                            <div class="col-md-4">
                                <div class="d-flex align-items-center gap-2">
                                    <span class="fs-5"><?php echo $cfg['icon']; ?></span>
                                    <h5 class="fw-bold mb-0" style="color: var(--text-primary, #0f172a);"><?php echo htmlspecialchars($p->user_name); ?></h5>
                                </div>
                                <span class="badge mt-1" style="background: #2563eb; color: #ffffff !important; font-weight: 600; font-size: 0.78rem;">
                                    👥 <?php echo htmlspecialchars($p->team_name ?: 'No Team'); ?>
                                </span>
                            </div>
                            ...
                            <div class="col-md-5">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    <div>
                                        <span class="text-secondary small me-1">Overall Score:</span>
                                        <span class="fs-4 fw-extrabold font-monospace <?php echo $cfg['score_color']; ?>"><?php echo number_format((float)$p->overall_score, 1); ?></span>
                                    ...
                                    <span class="badge px-2 py-1 fw-bold" style="<?php echo $bandStyle; ?> border-radius: 6px; font-size: 0.75rem;">
                                        <?php echo strtoupper(str_replace('_', ' ', $p->performance_band)); ?>
                                    </span>
                                </div>
                                <div class="progress bg-dark" style="height: 8px; border-radius: 4px;">
                                    <div class="progress-bar <?php echo $cfg['bar_bg']; ?>" style="width: <?php echo min(100, (float)$p->overall_score); ?>%;"></div>
                                </div>
                                <div class="mt-2 d-flex flex-wrap gap-3 text-secondary" style="font-size: 0.78rem;">
                                    <span>🎯 Target: <strong style="color: var(--text-primary, #0f172a);"><?php echo number_format((float)$p->target_score, 0); ?>%</strong></span>
                                    <span>⚡ Activity: <strong style="color: var(--text-primary, #0f172a);"><?php echo number_format((float)$p->activity_score, 0); ?>%</strong></span>
                                    <span>📞 Follow-up: <strong style="color: var(--text-primary, #0f172a);"><?php echo number_format((float)$p->followup_score, 0); ?>%</strong></span>
                                </div>
                            </div>
                        </div>
                    </div>
