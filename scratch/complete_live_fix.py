...
# CONFIRMED MySQL credentials from ProFreeHost cPanel screenshot:
DB_HOST = "sql212.ezyro.com"
DB_USER = "ezyro_42571719"
DB_PASS = "Axiorags@2026"
DB_NAME = "ezyro_42571719_raptor"

print("=== RAPTOR CRM LIVE FIX ===")
print(f"DB_HOST: {DB_HOST}")
print(f"DB_USER: {DB_USER}")
print(f"DB_PASS: {DB_PASS}")
print(f"DB_NAME: {DB_NAME}")

# ── Step 1: Upload corrected config.php ──────────────────────────────────────
config_php = f"""<?php
// Raptor CRM - ProFreeHost Production Config
// DB credentials from ProFreeHost cPanel MySQL panel
...
// ProFreeHost MySQL (sql212.ezyro.com)
...
print("\n[1/3] Uploading config.php via FTP...")
...
print(f"    [OK] config.php uploaded ({len(config_php)} bytes)")

# ── Step 2: Upload full_setup.php with correct credentials ───────────────────
setup_php = r"""<?php
...
echo "=== RAPTOR CRM DATABASE SETUP ===\n\n";
...
'role_permissions' => "CREATE TABLE IF NOT EXISTS role_permissions (
    ...
    scope ENUM('own','team','all') DEFAULT 'all',
...
echo "\n--- Seeding Roles ---\n";
$roles = [
    ['admin','System Administrator'], ['hr','HR Manager'], ['manager','Department Manager'],
    ['team_leader','Team Leader'], ['employee','Employee'], ['sales_person','Sales Person'],
    ['finance','Finance Officer'], ['analyst','Business Analyst'], ['ceo','Chief Executive Officer'],
    ['employer','Employer'],
];
foreach ($roles as [$n, $d]) {
    $pdo->prepare("INSERT INTO roles (role_name, description) VALUES (?,?) ON DUPLICATE KEY UPDATE description=VALUES(description)")->execute([$n,$d]);
    echo "[OK] Role: $n\n";
}
...
$perms = [
    ...
    ['leads.view','leads','view'],['leads.create','leads','create'],['leads.edit','leads','edit'],['leads.delete','leads','delete'],
    ['crm_leads.view','crm_leads','view'],['crm_leads.create','crm_leads','create'],['crm_leads.edit','crm_leads','edit'],
    ['followups.view','followups','view'],['followups.create','followups','create'],['followups.edit','followups','edit'],
    ['customers.view','customers','view'],['customers.create','customers','create'],['customers.edit','customers','edit'],
    ['communications.view','communications','view'],['communications.create','communications','create'],
    ['meetings.view','meetings','view'],['meetings.create','meetings','create'],['meetings.edit','meetings','edit'],
    ['tasks.view','tasks','view'],['tasks.create','tasks','create'],
    ['employees.view','employees','view'],
    ['reports.view','reports','view'],
    ['analytics.view','analytics','view'],
];
foreach ($perms as [$pn, $mod, $act]) {
    $pdo->prepare("INSERT INTO permissions (permission_name, module, action, description) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE module=VALUES(module)")->execute([$pn,$mod,$act,"$act $mod"]);
}
echo "[OK] " . count($perms) . " permissions seeded\n";

echo "\n--- Granting All Permissions to All Roles ---\n";
...
foreach ($allRoles as $r) {
    ...
    foreach ($allPerms as $pid) {
        $pdo->prepare("INSERT INTO role_permissions (role_id, permission_id, scope) VALUES (?,?,?) ON DUPLICATE KEY UPDATE scope=VALUES(scope)")->execute([$r['role_id'],$pid,$scope]);
    }
    echo "[OK] {$r['role_name']}: $scope access to all modules\n";
}

echo "\n--- Creating Default Users ---\n";
$defaultUsers = [
    ['Admin User', 'admin@raptor.local', 'Password123!', 'admin'],
    ['Test Employee', 'employee@raptor.local', 'Password123!', 'employee'],
    ['Test Manager', 'manager@raptor.local', 'Password123!', 'manager'],
];
foreach ($defaultUsers as [$name, $email, $pass, $roleName]) {
    $rid = $pdo->query("SELECT role_id FROM roles WHERE role_name='$roleName'")->fetchColumn();
    $exists = $pdo->query("SELECT COUNT(*) FROM users WHERE email='$email'")->fetchColumn();
    if (!$exists && $rid) {
        $pdo->prepare("INSERT INTO users (name, email, password_hash, role_id, status) VALUES (?,?,?,?,'active')")->execute([$name,$email,password_hash($pass,PASSWORD_BCRYPT),$rid]);
        echo "[OK] Created: $email / $pass (role: $roleName)\n";
    } else {
        echo "[SKIP] Already exists: $email\n";
    }
}

// Create employee record for employee user
$empUserId = $pdo->query("SELECT user_id FROM users WHERE email='employee@raptor.local'")->fetchColumn();
if ($empUserId) {
    $pdo->prepare("INSERT INTO employees (user_id, employee_code, job_title, department, status) VALUES (?,?,?,?,'active') ON DUPLICATE KEY UPDATE status='active'")->execute([$empUserId,'EMP001','Sales Executive','Sales']);
    echo "[OK] Employee record: EMP001\n";
}

echo "\n=== SETUP COMPLETE! ===\n";
$finalTables = $pdo->query('SHOW TABLES')->fetchAll(PDO::FETCH_COLUMN);
echo "Total tables: " . count($finalTables) . "\n";
echo "Tables: " . implode(', ', $finalTables) . "\n\n";
echo "Login URL: http://raptor.unaux.com/public/index.php?route=auth/login\n";
echo "Admin:    admin@raptor.local / Password123!\n";
echo "Employee: employee@raptor.local / Password123!\n";
"""

print("[2/3] Uploading full_setup.php via FTP...")
ftp.cwd("/htdocs/bin")
ftp.storbinary("STOR full_setup.php", io.BytesIO(setup_php.encode('utf-8')))
print(f"    [OK] full_setup.php uploaded")
ftp.quit()

print("[3/3] Triggering setup via HTTP...")
# AES cookie we already know
...
req = urllib.request.Request(
    "http://raptor.unaux.com/bin/full_setup.php",
